...
def apply_quant_element(src_val, quant_gm, mode, is_vector, dst_dtype, use_relu=False):
    m1, offset, sign, sat_bit = extract_quant_params(quant_gm, is_vector)
...
def process_quant(data_array, quant_array, src_dtype, dst_dtype, is_vector, use_relu):
    ...
    for j in range(cols):
        ...
        for i in range(rows):
            out[i, j] = apply_quant_element(data_array[i, j], q_param, mode, is_vector, dst_dtype, use_relu)
