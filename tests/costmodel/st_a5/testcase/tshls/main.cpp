...
void runTShlS(T scalar)
{
    ...
    TSHLS(dstTile, srcTile, scalar);

    constexpr uint64_t repeat = (static_cast<uint64_t>(rows) * cols + 63) / 64;
    pto::test::a5::ExpectLastVecTileOp({"vlds", "vshls", "vsts"}, repeat);
}

} // namespace

TEST(TShlS, uint32_1x512) { runTShlS<uint32_t, 1, 512>(1U); }
