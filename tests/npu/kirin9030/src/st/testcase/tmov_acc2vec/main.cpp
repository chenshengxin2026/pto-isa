...
void tmov_acc2vec_fb_quant_test(
    uint32_t M, uint32_t K, uint32_t N, uint32_t baseM = 0, uint32_t baseK = 0, uint32_t baseN = 0)
{
    uint32_t alignM = (baseM > 0) ? baseM : M;
    uint32_t alignK = (baseK > 0) ? baseK : K;
    uint32_t alignN = (baseN > 0) ? baseN : N;
    ...
    size_t bFileSize = alignK * alignN * sizeof(BType);
    size_t cFileSize = alignM * alignN * sizeof(CType);
    size_t FBQuantFileSize = alignN * sizeof(QuantType);
...
{
    // kirin9030: use int8_t to match a5 behavior, no padding needed for M=128, N=96
    tmov_acc2vec_fb_quant_test<2, int8_t, int8_t, int8_t, uint64_t, 3>(128, 32, 96);
}

TEST_F(TMOVTest, case_nz2nz_fb_quant_4)
{
    // kirin9030: use int8_t to match a5 behavior, pad M/K to 32-aligned (baseM=96, baseK=32)
    tmov_acc2vec_fb_quant_test<2, int8_t, int8_t, int8_t, uint64_t, 4>(80, 16, 128, 96, 32);
}

TEST_F(TMOVTest, case_nz2nz_sc_quant_1) { tmov_acc2vec_test<5, int16_t, aclFloat16, aclFloat16, 1>(48, 32, 80); }

TEST_F(TMOVTest, case_nz2nz_sc_quant_2) { tmov_acc2vec_test<5, uint16_t, int8_t, int8_t, 2>(96, 48, 128); }

TEST_F(TMOVTest, case_nz2nz_sc_quant_3) { tmov_acc2vec_test<5, int8_t, int8_t, int8_t, 3>(125, 64, 124, 128, 64, 128); }

TEST_F(TMOVTest, case_nz2nz_sc_quant_4)
{
    tmov_acc2vec_test<5, int8_t, aclFloat16, aclFloat16, 4>(61, 80, 93, 64, 80, 96);
}
...
{
    tmov_acc2vec_fb_quant_test<1, int8_t, uint16_t, uint16_t, uint64_t, 4>(60, 128, 64);
}

TEST_F(TMOVTest, case_nz2nd_fb_quant_5)
{
    tmov_acc2vec_fb_quant_test<1, int16_t, uint16_t, uint16_t, uint64_t, 5>(31, 128, 128);
}

TEST_F(TMOVTest, case_nz2nd_sc_quant_1) { tmov_acc2vec_test<4, int16_t, uint16_t, uint16_t, 1>(128, 48, 96); }

TEST_F(TMOVTest, case_nz2nd_sc_quant_2) { tmov_acc2vec_test<4, int8_t, uint16_t, uint16_t, 2>(60, 128, 64); }
