...
def gen_golden_data_tcmps(case_name, param):
    ...
    if dtype in (np.float16, np.float32):
        ...
        input2 = np.random.uniform(low=-dtype_info.max, high=dtype_info.max, size=[1]).astype(dtype)
    else:
    ...
    if param.mode == "CmpMode::EQ":
        golden = np.isclose(input1, input2[0], rtol=0, atol=1e-9)
...
def generate_case_name(param):
    ...
    return (
        f"TCMPSTest.case_{cmpmode_str}_{dtype_str}_{param.global_row}x{param.global_col}_"
...
if __name__ == "__main__":
    ...
    case_params_list = [
        ...
        tcmpsParams(np.float32, 32, 32, 32, 32, 32, 32, "CmpMode::NE"),
        tcmpsParams(np.float32, 2, 4096, 2, 4096, 2, 4096, "CmpMode::LT"),
        tcmpsParams(np.float32, 2, 4096, 2, 4096, 2, 4096, "CmpMode::NE"),
        tcmpsParams(np.float32, 2, 4096, 2, 4096, 2, 4096, "CmpMode::NE", is_nan=True),
        tcmpsParams(np.float32, 1, 64, 1, 64, 1, 64, "CmpMode::NE"),
        tcmpsParams(np.float32, 64, 64, 32, 32, 64, 64, "CmpMode::NE"),
        tcmpsParams(np.float16, 1, 64, 1, 64, 1, 64, "CmpMode::EQ"),
        tcmpsParams(np.float16, 8, 64, 8, 64, 8, 64, "CmpMode::NE"),
        tcmpsParams(np.float16, 32, 32, 32, 32, 32, 32, "CmpMode::LT"),
        tcmpsParams(np.float16, 32, 32, 32, 32, 32, 32, "CmpMode::GT"),
        tcmpsParams(np.float16, 16, 32, 16, 32, 16, 32, "CmpMode::GE"),
        tcmpsParams(np.float16, 128, 128, 64, 64, 128, 128, "CmpMode::LE"),
        tcmpsParams(np.float16, 32, 32, 32, 32, 32, 32, "CmpMode::NE", is_nan=True),
        tcmpsParams(np.float16, 2, 4096, 2, 4096, 2, 4096, "CmpMode::NE"),
        tcmpsParams(np.float16, 128, 128, 64, 64, 128, 128, "CmpMode::NE"),
    ]
