...
# Single-device multi-block FFN ReduceSum demo using the pto::comm collective
# API (pto::comm::TREDUCE).  grid-rows x grid-cols is the logical grid launched
# on one NPU.  Columns are model-parallel FFN shards; their fp32 down partials
# are reduce-summed per row through pto::comm::TREDUCE over a single-device fake
# HCCL context.
...
echo "=== Single-device Multi-block FFN pto::comm TREDUCE Demo ==="
...
cmake -DRUN_MODE=${RUN_MODE} -DSOC_VERSION=${SOC_VERSION} \
      ...
      ..
make -j16 distributed_ffn_grid_comm_reducesum
...
echo ""
echo "=== Running Single-device Multi-block FFN pto::comm TREDUCE ==="
export N_RANKS=${N_RANKS}
export FFN_GRID_DATA_DIR="${SCRIPT_DIR}/out"
./distributed_ffn_grid_comm_reducesum --device-id "${DEVICE_ID}"
