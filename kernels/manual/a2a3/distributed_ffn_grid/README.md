...
| `../../../../include/pto/npu/a2a3/grid_cce_intrinsic.hpp` | Grid CCE facades: `copy_l1_to_neighbor_l1` / `copy_l1_to_group` for outbound unified-L1 transfers, `sync_hscb` for peer publication, `wait_ipc_scb` for local blocking waits, and `mov_ubuf_group` for group reduction. The A3 mock represents L1 ranges with GM windows. |
...
```text
control + record | unicast receive rings | optional broadcast receive ring + lanes | producer staging slot
                                                                                     [SlotStride bytes]
```
...
`GRID_TBROADCAST` stages once, then uses `copy_l1_to_group` for an affine group arena or per-member `copy_l1_to_neighbor_l1` otherwise. `GRID_TREDUCE_GROUP_IMPL` continues to lower SUM/MAX/MIN to `mov_ubuf_group`; it is a genuine N→1 fan-in and is distinct from the peer-id TPOP + TADD + TPUSH relay.

### fp32 reduction

The reduce slot carries fp32 `[T, H]`, so `FFN_SLOT_BYTES = T * H * 4`. This keeps `downPartial`, `yOutput`, and `golden.bin` in fp32 for direct tolerance-based comparison. The ReduceSum reduce is H-chunked (`FFN_RS_REDUCE_SLOT_COUNT = kHSegs` = 7, one slot per H-segment): the `treduce` example folds the segment-h partials with `GRID_TREDUCE_GROUP_IMPL` at the sink, while the `tpush` example relays them hop-by-hop with peer-id TPOP + TADD + TPUSH. Its EAST and SOUTH launches intentionally share one physical channel; the last H segment closes the first turn and the next launch resumes from the relayed count.
