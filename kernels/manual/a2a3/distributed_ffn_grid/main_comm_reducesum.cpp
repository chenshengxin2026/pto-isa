...
// Single-device multi-block FFN driver, ReduceSum combine via pto::comm::TREDUCE.
//
// gridRows*gridCols blocks form a logical grid on one NPU.  A compute kernel
// publishes each cell's fp32 down partial [T,H] into its HCCL window slot; a
// host stream-sync barrier follows; then a collective kernel runs one
// pto::comm::TREDUCE(Sum) per row (root = the row block) over the row's column
// window slots, producing the final fp32 [T,H] output per row.  The HCCL
// context is the same single-device fake context as the GridPipe demo
// (windowsIn[i] = base + i*FFN_GRID_WINDOW_BYTES, rankId = 0), so HcclRemotePtr
// resolves remote rank slots to per-cell sub-buffers on the same device.
...
// Build the single-device fake HcclDeviceContext: each cell owns one window
// slot, windowsIn[i] = base + i*FFN_GRID_WINDOW_BYTES, rankId = 0.  This is the
// same mock the GridPipe demo uses; here the slots hold the per-cell partials
// the collective reduces.
...
static bool VerifyOutput(DeviceResources &r)
{
    ...
    std::cout << "[INFO] ResultCmp single-device pto::comm TREDUCE output vs golden:" << std::endl;
    return PtoTestCommon::ResultCmp(golden, outHost.data(), 0.001f);
}

static void Cleanup(DeviceResources &r)
{
    void *buffers[] = {r.fake_hccl_ctx_dev, r.windows_dev,     r.w_gate_dev,      r.w_up_dev,  r.w_down_dev,
                       r.x_dev,             r.gate_partial_dev, r.up_partial_dev, r.hidden_dev, r.down_partial_dev,
                       r.y_output_dev};
    for (void *p : buffers) {
        if (p) {
            aclrtFree(p);
    ...
    r.x_dev = r.gate_partial_dev = r.up_partial_dev = r.hidden_dev = r.down_partial_dev = r.y_output_dev = nullptr;
    if (r.stream) {
...
static bool RunSingleDevice()
{
    ...
    // Phase 1: each cell computes its fp32 [T,H] down partial and stores it into
    // its window slot.
    launchFfnCommReduceSumComputeKernel(
        ...
        reinterpret_cast<uint8_t *>(r.hidden_dev), reinterpret_cast<uint8_t *>(r.down_partial_dev), FFN_GRID_ROWS,
    ...
    // Phase 2: one TREDUCE(Sum) per row over the row's column window slots.
    launchFfnCommReduceSumCollectiveKernel(
        reinterpret_cast<uint8_t *>(r.ffts), reinterpret_cast<uint8_t *>(r.windows_dev),
...
int main(int argc, char **argv)
{
    ...
    std::cout << "\n================================================================" << std::endl;
    std::cout << "  Single-device multi-block FFN pto::comm TREDUCE demo" << std::endl;
    ...
    std::cout << "  Mode: row=data-parallel, col=model-parallel; combine via pto::comm::TREDUCE(Sum)" << std::endl;
    std::cout << "================================================================" << std::endl;

    bool ok = RunSingleDevice();
    std::cout << (ok ? "[SUCCESS] Single-device multi-block FFN pto::comm TREDUCE PASS." :
                       "[FAILED] Single-device multi-block FFN pto::comm TREDUCE FAILED.")
              << std::endl;
