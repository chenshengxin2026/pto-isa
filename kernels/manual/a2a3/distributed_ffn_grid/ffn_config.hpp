...
//   + 2 * GroupMax * kScbLineStride (per-source ready lanes + free lanes)
...
// header(1216) + 0 + (BcastSlotCount*SlotStride + 2*Group*kBcastLaneStride)
// + one isolated producer staging slot,
...
constexpr int FFN_NCUT_WIN_P1 = FFN_NCUT_GRID_SLOT_REGION_OFFSET +
                                ...
                                FFN_NCUT_BCAST_SLOTS_P1 * FFN_NCUT_SLOT_BYTES_P1 +
                                2 * FFN_NCUT_GROUP_P1 * FFN_NCUT_LANE_STRIDE + FFN_NCUT_SLOT_BYTES_P1;
constexpr int FFN_NCUT_WIN_P2 = FFN_NCUT_GRID_SLOT_REGION_OFFSET +
                                ...
                                FFN_NCUT_BCAST_SLOTS_P2 * FFN_NCUT_SLOT_BYTES_P2 +
                                2 * FFN_NCUT_GROUP_P2 * FFN_NCUT_LANE_STRIDE + FFN_NCUT_SLOT_BYTES_P2;
...
// The reduce runs as TWO launches over ONE window: phase B reduces EAST, phase C
// reduces SOUTH.  They intentionally share ONE physical channel to exercise the
// time-division MPSC relay-count protocol: phase B's final TPUSH publishes CLOSE;
// phase C sends its producer id, receives the surviving ready/cons baselines, and
// continues the absolute sequence on the same ring without clearing an SCB.
