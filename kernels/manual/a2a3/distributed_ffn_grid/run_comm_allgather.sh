...
# Single-device multi-block FFN AllGather demo using the pto::comm collective
# API (pto::comm::TGATHER).  grid-rows x grid-cols is the logical grid launched
# on one NPU.  Columns are model-parallel FFN shards; every cell TGATHERs the
# row's hidden shards over a single-device fake HCCL context, then down-projects
# its own output-H shard.
...
echo "=== Single-device Multi-block FFN pto::comm TGATHER Demo ==="
...
cmake -DRUN_MODE=${RUN_MODE} -DSOC_VERSION=${SOC_VERSION} \
      ...
      ..
make -j16 distributed_ffn_grid_comm_allgather
...
echo ""
echo "=== Running Single-device Multi-block FFN pto::comm TGATHER ==="
export N_RANKS=${N_RANKS}
export FFN_GRID_DATA_DIR="${SCRIPT_DIR}/out"
./distributed_ffn_grid_comm_allgather --device-id "${DEVICE_ID}"
