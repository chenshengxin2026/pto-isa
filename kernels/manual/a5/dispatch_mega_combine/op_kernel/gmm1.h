...
public:
    AICORE inline void Init(
        GM_ADDR weight1GM, GM_ADDR scale1GM, GM_ADDR workspaceGM, const __gm__ MegaMoeTilingData* tilingData);
...
private:
    ...
    {
        CrossCoreWaitFlag<0x2>(MegaMoeD2CHardFlagId(groupIdx));
    ...
    AICORE inline void SetC2VReady() const;
...
AICORE inline void Gmm1::SetC2VReady() const
{
    pipe_barrier(PIPE_ALL);
    CrossCoreSetFlag<0x2, PIPE_FIX>(MEGA_MOE_C2V_HARD_FLAG_BASE);
}

AICORE inline void Gmm1::Process()
{
    if ASCEND_IS_AIV {
        return;
    }
    ...
    for (uint32_t groupIdx = 0; groupIdx < expertPerRank_; ++groupIdx) {
        ...
        const uint32_t currentM =
            MoeClipCurrentM(currentMRaw, groupBase, maxOutputSize_); // 当前expert要处理的token的总行数
        ...
        if (groupIdx + 1U == MoeSwigluSegmentEndExpert(expertPerRank_, segmentIdx)) {
