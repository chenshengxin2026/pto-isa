...
class FrontReorderVmsSort : public FrontReorderContext {
public:
        ...
        const __gm__ MegaMoeTilingData* tilingData)
    {
        ...
        InitCommonSortTiling(front);
        InitCommonCoreIdx();
        InitCommonWorkspacePtrs(front, true);
...
private:
    ...
    {
        return tilingData_->frontReorderTiling.frontCase == kFrontCaseOneCoreDynamic;
    }

    AICORE inline uint32_t SingleSortTileLength() const
    {
        const uint32_t lastCoreElems = sortLastCorePerLoopElems_ == 0U ? routeElems_ : sortLastCorePerLoopElems_;
        return static_cast<uint32_t>(alignUp(lastCoreElems, static_cast<uint32_t>(sizeof(int32_t))));
    }
    ...
    {
        return coreIdx_ == kFrontSingleCoreSortOwnerCore && routeElems_ != 0U && sortNum <= kFrontSortMaxElems &&
        ...
        uint32_t perListElements, uint32_t lastListElements, uint64_t srcWorkspaceBytes, bool extractToFinal) const
    {
        ...
        return !extractToFinal || state.allRemain == routeElems_;
    }
        ...
        uint64_t mergedUb, uint32_t curLoopSortedNum, uint64_t dstWorkspaceBytes, bool extractToFinal) const
    {
        if (extractToFinal) {
            ...
            pto::PtoSetWaitFlag<PIPE_V, PIPE_MTE3>();
            PtoStoreVector<int32_t>(frontExpandedExpertPtr_ + listState.outOffset, 0U, curLoopSortedNum);
            PtoStoreVector<int32_t>(
                frontExpandDstToSrcPtr_ + listState.outOffset, SortOutPayloadUb(loopState.activeListNum, perListElems),
                curLoopSortedNum);
        ...
        bool extractToFinal) const
    {
        ...
        while (listState.allRemain > 0U) {
            ...
            const uint32_t perListElems = sortOutLoopMaxElems_;
    ...
    AICORE inline void BuildOneCoreVmsProcess() const
    {
        ...
        uint32_t srcWsIndex = 0U;
        while (listNum > 1U) {
            // kFrontMergeOutMaxFanIn=4路归并
            const uint32_t loops = (listNum + kFrontMergeOutMaxFanIn - 1U) / kFrontMergeOutMaxFanIn;
    ...
    AICORE inline void BuildVbsSortRuns() const
    {
        ...
        const bool isLastSortCore = coreIdx_ == sortNeedCoreNum_ - 1U;
        const uint32_t coreLoops = isLastSortCore ? sortLastCoreLoops_ : sortPerCoreLoops_;
        ...
        for (uint32_t loop = 0U; loop < coreLoops; ++loop) {
