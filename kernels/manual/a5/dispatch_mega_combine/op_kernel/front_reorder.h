...
inputValueUb    原始 expertId 数组，int32
inputPayloadUb  原始 routeIdx 数组，uint32，通常是 0,1,2... 或 loopStart...
packedSortUb    输出 packed records 的 UB 地址
mergeTmpUb      TMRGSORT 临时 UB
sortKeyUb       float sort key 临时 UB
elemNum         有效元素数
...
constexpr uint32_t kFrontCountSignalStrideElems = 128U;
constexpr int32_t kFrontCountSignalBias = 0x800000;
...
AICORE inline FrontCoreRowLoopView FrontGetCoreRowLoopView(
    const FrontReorderContext& op, const FrontRowSplitCoreTiling& tiling, uint32_t perLoopRows, uint32_t lastLoopRows,
    uint32_t explicitRowLoops = 0U)
{
...
AICORE inline FrontCoreRowLoopView FrontGetCoreRowLoopView(
    const FrontReorderContext& op, const FrontRowSplitLoopTiling& tiling)
{
...
AICORE inline FrontCoreRowLoopView FrontGetCoreRowLoopView(
    const FrontReorderContext& op, const FrontGatherQuantTiling& tiling)
{
    if (op.coreIdx_ >= tiling.needCoreNum) {
        return FrontCoreRowLoopView{};
    }
...
AICORE inline uint32_t FrontAlignOneBlockByteCeil(uint64_t value)
{
...
AICORE inline uint32_t FrontSrcToDstPerLoopMaxRows(uint32_t coreNum)
{
    ...
    if (AtlasA5::UB_SIZE <= reservedBytes) {
        return 0U;
    }
    return static_cast<uint32_t>((AtlasA5::UB_SIZE - reservedBytes) / (kFrontSortAlignElement * 2U) / 2U);
}
...
{
    return AtlasA5::UB_SIZE > colSize + scaleSize + 32U * 4U * 4U && problemK == kFrontFullLoadHLimit;
}

AICORE inline void FrontApplyGatherQuantOneLoop(
    FrontGatherQuantTiling& tiling, uint32_t perCoreRows, uint32_t onceRowSize, bool forceSplit)
{
    ...
    uint64_t scaleSize)
{
    ...
    uint32_t basePerLoopMaxRows =
        FrontAlignOneBlockByteCeil((AtlasA5::UB_SIZE - totalColSize - scaleSize) / sizeof(int32_t)) / 4U;
    if (problemK < 6144U) {
        basePerLoopMaxRows =
            FrontAlignOneBlockByteCeil((AtlasA5::UB_SIZE - colSize - scaleSize) / sizeof(int32_t)) / 4U;
    } else if (perCoreRows < basePerLoopMaxRows) {
        baseMaxCols = FrontAlignOneBlockByteCeil(AtlasA5::UB_SIZE - rowSize - scaleSize) / kFrontDynamicQuantColsBuffer;
    }
    ...
    uint32_t routeElems, uint32_t coreNum, uint32_t problemK)
{
    ...
    if (AtlasA5::UB_SIZE > colSize + scaleSize + 32U * 4U * 3U) {
        onceRowSize =
            static_cast<uint32_t>((AtlasA5::UB_SIZE - colSize - scaleSize - 32U * 4U * 3U) / (sizeof(int32_t) * 4U));
    ...
    if (rowSize + colSize + scaleSize < AtlasA5::UB_SIZE || ifOneLoop) {
        FrontApplyGatherQuantOneLoop(tiling, perCoreRows, onceRowSize, ifOneLoop);
        return tiling;
    }
...
{
    return coreLoop.coreRows != 0U && coreLoop.perLoopRows != 0U && requiredUbBytes <= AtlasA5::UB_SIZE;
}
...
template <typename InputElement>
AICORE inline void FrontDynamicQuantRowToUb(
...
AICORE inline void FrontStoreQuantPackedRow(__gm__ int8_t* offsetAPtr, uint32_t dstRow, uint32_t k, uint64_t outUb)
{
    PtoStoreVector<int8_t>(offsetAPtr + FrontPackedRowOffset(dstRow, k), outUb, FrontPackedRowStride(k));
}
...
AICORE inline uint64_t FrontEndCountRowBytes(const FrontReorderContext& op)
{
...
AICORE inline uint64_t FrontEndPrefixUb(const FrontReorderContext& op)
{
    return FrontEndCountRowUb() + FrontEndCountRowBytes(op);
}

AICORE inline uint64_t FrontEndTputUb(const FrontReorderContext& op)
{
    return FrontEndPrefixUb(op) + FrontEndCountRowBytes(op);
}

AICORE inline uint64_t FrontEndCumsumUb(const FrontReorderContext& op)
{
    return FrontEndTputUb(op) + FrontEndCountRowBytes(op);
}

AICORE inline uint64_t FrontEndCountExchangeRequiredUbBytes(const FrontReorderContext& op)
{
    return FrontEndCumsumUb(op) + static_cast<uint64_t>(op.rankSize_) * op.expertPerRank_ * sizeof(int32_t);
}

AICORE inline __gm__ int32_t* FrontEndTokenPerExpertRow(const FrontReorderContext& op, uint32_t srcRank)
{
...
AICORE inline bool FrontEndPostprocessEnabled(const FrontReorderContext& op)
{
    return op.rankSize_ != 0U && op.expertNumAligned_ != 0U && op.expertPerRank_ != 0U &&
           op.expertNumAligned_ <= kFrontEndMaxExpertNumAligned &&
           (op.expertPerRank_ == 8U || op.expertPerRank_ == 16U) &&
           FrontEndCountExchangeRequiredUbBytes(op) <= AtlasA5::UB_SIZE;
}

/* 本 rank 把自己的整行 localTokenPerExpert[expertNumAligned]
写到每个 peer 的 tokenPerExpert[rank, :]
 */
AICORE inline void FrontEndPublishCountRowsToPeers(const FrontReorderContext& op)
{
    for (uint32_t dstRank = op.coreIdx_; dstRank < op.rankSize_; dstRank += op.coreNum_) {
        ...
        PtoAddScalarUb<int32_t>(
            FrontEndCountRowUb(), FrontEndCountRowUb(), op.expertNumAligned_,
            kFrontCountSignalBias); // data as flag
        pipe_barrier(PIPE_ALL);
        pto::PtoSetWaitFlag<PIPE_V, PIPE_MTE3>();
...
AICORE inline void FrontEndWaitCountRowSignal(const FrontReorderContext& op, uint32_t srcRank)
{
    if (srcRank == op.rank_) {
        return;
    }

    using SignalGlobal = pto::GlobalTensor<int32_t, FrontEndShapeDyn, FrontEndStrideDyn, pto::Layout::ND>;
    const uint32_t signalNum = static_cast<uint32_t>(ceilDiv(op.expertNumAligned_, kFrontCountSignalStrideElems));
    FrontEndShapeDyn signalShape(1, 1, 1, 1, signalNum);
    FrontEndStrideDyn signalStride(
        kFrontCountSignalStrideElems, kFrontCountSignalStrideElems, kFrontCountSignalStrideElems,
        kFrontCountSignalStrideElems, kFrontCountSignalStrideElems);
    SignalGlobal signal(FrontEndTokenPerExpertRow(op, srcRank), signalShape, signalStride);
    pto::comm::TWAIT(signal, 0, pto::comm::WaitCmp::NE);
    MegaMoeDcciGmRange(static_cast<__gm__ void*>(FrontEndTokenPerExpertRow(op, srcRank)), FrontEndCountRowBytes(op));
}

AICORE inline void FrontEndRestoreCountRowAndBuildPreSum(const FrontReorderContext& op, uint32_t srcRank)
{
    __gm__ int32_t* srcRow = FrontEndTokenPerExpertRow(op, srcRank);
    FrontEndWaitCountRowSignal(op, srcRank);

    if (srcRank != op.rank_) {
        PtoLoadVector<int32_t>(FrontEndCountRowUb(), srcRow, op.expertNumAligned_);
        pto::PtoSetWaitFlag<PIPE_MTE2, PIPE_V>();
        PtoAddScalarUb<int32_t>(
            FrontEndCountRowUb(), FrontEndCountRowUb(), op.expertNumAligned_, -kFrontCountSignalBias);
        pipe_barrier(PIPE_ALL);
        pto::PtoSetWaitFlag<PIPE_V, PIPE_MTE3>();
 ...
 */
AICORE inline void FrontEndBuildCountExchangeAndPreSum(const FrontReorderContext& op)
{
...
AICORE inline void FrontEndBuildCumsumForPitch(const FrontReorderContext& op)
{
    ...
    const uint32_t localBegin = op.rank_ * op.expertPerRank_;
    CumsumTile cumsumTile(op.rankSize_, op.expertPerRank_);
    pto::TASSIGN(cumsumTile, FrontEndCumsumUb(op));

    FrontEndShapeDyn loadShape(1, 1, 1, op.rankSize_, op.expertPerRank_);
    ...
    for (uint32_t srcRank = 1U; srcRank < op.rankSize_; ++srcRank) {
        PtoAddUb<int32_t>(
            ...
            FrontEndCumsumUb(op) + static_cast<uint64_t>(srcRank) * Pitch * sizeof(int32_t),
            FrontEndCumsumUb(op) + static_cast<uint64_t>(srcRank - 1U) * Pitch * sizeof(int32_t), op.expertPerRank_);
        pipe_barrier(PIPE_ALL);
    }

    pto::PtoSetWaitFlag<PIPE_V, PIPE_MTE3>();
    FrontEndShapeDyn storeShape(1, 1, 1, op.rankSize_, op.expertPerRank_);
    FrontEndStrideDyn storeStride(
        op.rankSize_ * op.expertPerRank_, op.rankSize_ * op.expertPerRank_, op.rankSize_ * op.expertPerRank_,
        op.expertPerRank_, 1);
...
输出：
   ...
   */
AICORE inline void FrontEndBuildCumsumAndExpertTokenNums(const FrontReorderContext& op)
{
    if (op.coreIdx_ == 0U) {
        if (op.expertPerRank_ == 8U) {
            FrontEndBuildCumsumForPitch<8U>(op);
        } else if (op.expertPerRank_ == 16U) {
            FrontEndBuildCumsumForPitch<16U>(op);
        }
    }

    if (op.coreIdx_ == 0U) {
        PtoLoadVector<int32_t>(
            FrontEndPrefixUb(op), op.cumsumMMPtr_ + static_cast<uint64_t>(op.rankSize_ - 1U) * op.expertPerRank_,
            op.expertPerRank_);
        pto::PtoSetWaitFlag<PIPE_MTE2, PIPE_MTE3>();
        PtoStoreVector<int32_t>(op.expertTokenNumsPtr_, FrontEndPrefixUb(op), op.expertPerRank_);
        pto::PtoSetWaitFlag<PIPE_MTE3, PIPE_S>();
    }
}

AICORE inline void FrontRunEndPostprocess(const FrontReorderContext& op)
{
    if (!FrontEndPostprocessEnabled(op)) {
        return;
    }
    FrontEndBuildCountExchangeAndPreSum(op);
    FrontEndBuildCumsumAndExpertTokenNums(op);
    pto::SYNCALL<pto::SyncCoreType::AIVOnly>();
}

AICORE inline void FrontStoreExpertTokenOutCountSlice(
    const FrontReorderContext& op, int32_t firstExpertId, uint32_t copyLength, uint64_t countUb,
...
AICORE inline void FrontBuildExpertTokenOut(const FrontReorderContext& op)
{
...
AICORE inline void FrontPrepareSrcToDstAssist(
    const FrontReorderContext& op, uint64_t assistUb, const FrontSrcToDstTiling& tiling)
{
...
AICORE inline void FrontComputeSrcToDstRows(
    uint32_t loopIndex, uint32_t perLoopRows, uint32_t currentLoopRows, uint64_t outputUb, uint64_t assistUb)
{
    ...
    for (uint32_t loop = 0U; loop < loops; ++loop) {
        PtoAddScalarUb<int32_t, kFrontSrcToDstAssistElems>(
            ...
            static_cast<int32_t>(loopIndex * perLoopRows + loop * kFrontSrcToDstAssistIndexElems));
    }
    pipe_barrier(PIPE_ALL);
...
AICORE inline void FrontCopyOutSrcToDstRows(
    const FrontReorderContext& op, uint32_t currentLoopRows, uint64_t inputUb, uint64_t outputUb, bool hasNextLoop)
{
 ...
 */
AICORE inline void FrontBuildSrcToDst(const FrontReorderContext& op)
{
    const FrontSrcToDstTiling tiling = FrontBuildSrcToDstTiling(op.routeElems_, op.coreNum_);
...
template <typename InputElement>
AICORE inline void FrontDynamicQuantRow(
    const FrontReorderContext& op, __gm__ InputElement* xRow, const FrontGatherQuantUbPlan<InputElement>& ubPlan)
{
    FrontDynamicQuantRowToUb<InputElement>(
        xRow, op.problemK_, ubPlan.rawUb, ubPlan.fp32Ub, ubPlan.tmpUb, ubPlan.outUb, ubPlan.scaleUb);
}

template <typename InputElement>
AICORE inline void FrontCopyInGatherExpandedRowIdx(
    const FrontReorderContext& op, uint32_t coreBase, uint32_t loopIndex, uint32_t perLoopRows,
    uint32_t currentLoopRows, uint64_t indexUb)
{
    PtoLoadVector<int32_t>(indexUb, op.expandedRowIdxPtr_ + coreBase + loopIndex * perLoopRows, currentLoopRows);
    pto::PtoSetWaitFlag<PIPE_MTE2, PIPE_S>();
}

template <typename InputElement>
AICORE inline void FrontStoreGatherQuantPackedRow(const FrontReorderContext& op, uint32_t dstRow, uint64_t outUb)
{
    FrontStoreQuantPackedRow(op.offsetAPtr_, dstRow, op.problemK_, outUb);
}

template <typename InputElement>
AICORE inline void FrontCopyOutGatherQuant1H(
    const FrontReorderContext& op, uint32_t coreBase, uint32_t loopIndex, uint32_t perLoopRows,
    uint32_t currentLoopRows, const FrontGatherQuantUbPlan<InputElement>& ubPlan)
{
    if (currentLoopRows == 0U || op.topK_ == 0U) {
        return;
    }
    uint32_t initialRow = coreBase + perLoopRows * loopIndex;
    uint32_t curLoopRow = 0U;
    const uint32_t currentLoopStartRow = initialRow / op.topK_;
    const uint32_t currentLoopLastRow = (initialRow + currentLoopRows - 1U) / op.topK_;

    for (uint32_t row = currentLoopStartRow; row <= currentLoopLastRow && row < op.problemM_; ++row) {
        // 1 token*problemK_ 一起做量化
        FrontDynamicQuantRow<InputElement>(
            op, reinterpret_cast<__gm__ InputElement*>(op.xPtr_) + static_cast<uint64_t>(row) * op.problemK_, ubPlan);

        bool rowStored = false;
        // 1 token*problemK_ 条路由按照dstRow写到目的地址
        while (curLoopRow < currentLoopRows && initialRow / op.topK_ == row) {
            const int32_t outIndex = PtoGetValue<int32_t>(ubPlan.indexUb, curLoopRow);
            ++curLoopRow;
            ++initialRow;
            if (outIndex < 0 || static_cast<uint32_t>(outIndex) >= op.routeElems_) {
                continue;
            }
            FrontStoreGatherQuantPackedRow<InputElement>(op, static_cast<uint32_t>(outIndex), ubPlan.outUb);
            rowStored = true;
        }
        if (rowStored) {
...
template <typename InputElement>
AICORE inline void FrontBuildGatherDynamicQuant(const FrontReorderContext& op)
{
    const FrontGatherQuantTiling tiling = FrontBuildGatherQuantTiling(op.routeElems_, op.coreNum_, op.problemK_);
    const FrontCoreRowLoopView coreLoop = FrontGetCoreRowLoopView(op, tiling);
    const bool inputsValid = op.routeElems_ != 0U && op.topK_ != 0U && op.problemK_ % UB_ALIGN == 0U;
    if (inputsValid && FrontPostSortGatherQuantEnabled(tiling, op.problemK_) && coreLoop.active) {
        ...
        if (FrontGatherQuantCoreLoopReady(coreLoop, ubPlan.requiredBytes)) {
            for (uint32_t loop = 0U; loop < coreLoop.rowLoops; ++loop) {
                ...
                FrontCopyOutGatherQuant1H<InputElement>(
                    op, coreLoop.coreBase, loop, coreLoop.perLoopRows, currentLoopRows, ubPlan);
            }
        }
    }
    pipe_barrier(PIPE_ALL);
...
template <typename InputElement>
AICORE inline void FrontRunPostSortPipeline(const FrontReorderContext& op)
{
    ...
    FrontBuildGatherDynamicQuant<InputElement>(op);
    FrontRunEndPostprocess(op);
}
