...
def round_nearest_even_to_int8(x: np.ndarray) -> np.ndarray:
    return np.rint(np.clip(x, -128.0, 127.0)).astype(np.int8)


def quantize_fp32_via_fp16_cast_rint_to_int8(x: np.ndarray) -> np.ndarray:
    return round_nearest_even_to_int8(fp32_to_fp16_value(x))
...
def quantize_swiglu_rows_to_int8(x: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    quant = quantize_fp32_via_fp16_cast_rint_to_int8(quant_input)
    return quant, scale[:, 0].astype(np.float32)
...
def make_weight1(rank: int, args: argparse.Namespace) -> np.ndarray:
    if args.case_mode == "zero":
...
def make_weight2(rank: int, args: argparse.Namespace) -> np.ndarray:
    if args.case_mode == "zero":
...
def matmul_weight1(lhs: np.ndarray, dst_rank: int, local_expert: int, args: argparse.Namespace) -> np.ndarray:
    ...
    spec = PeriodicWeightSpec(args.k, args.n, args.n, offset, 7, 3)
    return matmul_periodic_weight(lhs, spec)
...
def matmul_weight2(lhs: np.ndarray, dst_rank: int, local_expert: int, args: argparse.Namespace) -> np.ndarray:
    ...
    spec = PeriodicWeightSpec(hidden, args.k, args.k, offset, 9, 4)
    return matmul_periodic_weight(lhs, spec)


def compute_outputs_naive(data: GoldenInputs, args: argparse.Namespace) -> list[np.ndarray]:
    xs = data.xs
    weight1_nd = data.weight1_nd
    weight2_nd = data.weight2_nd
    scale1_origin = data.scale1_origin
    scale2_origin = data.scale2_origin
    expert_idx_list = data.expert_idx_list
            ...
            for src_rank in range(args.world_size):
                for token_idx in range(args.m):
                    for topk_idx in range(args.topk):
                        ...
                        x_token = xs[src_rank][token_idx:token_end, :]
...
def compute_outputs_batch(data: GoldenInputs, args: argparse.Namespace) -> list[np.ndarray]:
    ...
    outputs = [np.zeros((args.m, args.k), dtype=np.float32) for _ in range(args.world_size)]
    chunk_rows = max(1, int(args.golden_chunk_rows))
    ...
    for dst_rank in range(args.world_size):
        for local_expert in range(args.experts):
            ...
            scale2_origin_row = scale2_origin[dst_rank][local_expert][None, :]
            for start in range(0, len(routes), chunk_rows):
                chunk_end = start + chunk_rows
                ...
                if args.case_mode == "cpu-golden":
                    product1 = matmul_weight1(qx, dst_rank, local_expert, args)
                else:
                    product1 = matmul_int8_exact(qx, weight1_nd[dst_rank][local_expert], args.k)
                ...
                if args.case_mode == "cpu-golden":
                    product2 = matmul_weight2(qswiglu, dst_rank, local_expert, args)
                else:
                    product2 = matmul_int8_exact(qswiglu, weight2_nd[dst_rank][local_expert], args.n // 2)
...
def build_rank_case(rank: int, case: RankCaseContext) -> None:
    out_dir = case.out_dir
    inputs = case.inputs
    rank_file_sizes = case.rank_file_sizes
    reuse_static_rank_files = case.reuse_static_rank_files
    write(out_dir / f"rank{rank}_x.bin", fp32_to_bf16_bits(inputs.xs[rank]))
    write_static_or_reuse(
        out_dir / f"rank{rank}_weight1.bin",
        rank_file_sizes["weight1"],
        reuse_static_rank_files,
        lambda: pack_expert_weights_to_zn(inputs.weight1_nd[rank]),
    )
    write_static_or_reuse(
        out_dir / f"rank{rank}_weight2.bin",
        rank_file_sizes["weight2"],
        reuse_static_rank_files,
        lambda: pack_expert_weights_to_zn(inputs.weight2_nd[rank]),
    )
    write(out_dir / f"rank{rank}_expert_idx.bin", inputs.expert_idx_list[rank].astype(np.int32))
    write_static_or_reuse(
        out_dir / f"rank{rank}_scale1.bin",
        rank_file_sizes["scale1"],
        reuse_static_rank_files,
        lambda: pack_scale_fp32_to_int64(inputs.scale1_origin[rank]),
    )
    write_static_or_reuse(
        out_dir / f"rank{rank}_scale2.bin",
        rank_file_sizes["scale2"],
        reuse_static_rank_files,
        lambda: pack_scale_fp32_to_int64(inputs.scale2_origin[rank]),
    )
    write(out_dir / f"rank{rank}_probs.bin", inputs.probs_list[rank].astype(np.float32))
    write(out_dir / f"rank{rank}_expected_out.bin", fp32_to_fp16_bits(case.expected_out_list[rank]))
...
def main() -> None:
    ...
    if skip_weight_arrays:
        ...
        weight1_nd = [make_weight1(rank, args) for rank in range(args.world_size)]
        weight2_nd = [make_weight2(rank, args) for rank in range(args.world_size)]
    scale1_origin = [
    ...
    inputs = GoldenInputs(xs, weight1_nd, weight2_nd, scale1_origin, scale2_origin, expert_idx_list, probs_list)
    expected_out_list = compute_outputs(inputs, args)
    ...
    rank_case = RankCaseContext(out_dir, inputs, expected_out_list, rank_file_sizes, reuse_static_rank_files)
    for rank in range(args.world_size):
        build_rank_case(rank, rank_case)
