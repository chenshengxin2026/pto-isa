...
template <typename Pipe>
AICORE inline int OpenDynamicOutgoingBinding(
    Pipe& pipe, uint32_t consId, int prodChan, uint32_t maxSpins)
{
    const int peerCount = BindQueuePeerCount(pipe);
    const int selfBlockId = BlockIdFromCoord(pipe.coord, pipe.shape);
    if (peerCount < 0 || selfBlockId < 0 || selfBlockId >= peerCount ||
        consId >= static_cast<uint32_t>(peerCount)) {
        grid_mock::MockSetFault(FaultWord(pipe.closeScb[0]), grid_mock::kFaultBindProtocol);
        return kGridInvalidChan;
    }
    ...
    grid_cce_detail::write_local_word(
        localResponse + kGridBindQueueResponseCommitWord, kGridBindPending);
    grid_cce_detail::write_local_word(
        localResponse + kGridBindQueueResponseTokenWord, kGridBindPending);
    grid_cce_detail::write_local_word(
        localResponse + kGridBindQueueResponseReadyWord, kGridBindPending);
    grid_cce_detail::write_local_word(
        localResponse + kGridBindQueueResponseConsChanWord, kGridBindPending);
    GridPublishFence();
    ...
    uint32_t completeWord = kGridBindPending;
    if (!WaitL1WordEqual(
            localResponse + kGridBindQueueResponseCommitWord, token, maxSpins, completeWord)) {
        grid_mock::MockSetFault(FaultWord(pipe.closeScb[0]), grid_mock::kFaultBindResponseTimeout);
        return kGridInvalidChan;
    }
    ...
    if (responseToken != token || peerConsChanWord == 0 ||
        peerConsChanWord > static_cast<uint32_t>(Pipe::ChanCount)) {
        grid_mock::MockSetFault(FaultWord(pipe.closeScb[0]), grid_mock::kFaultBindProtocol);
        return kGridInvalidChan;
    }
    const int peerConsChan = static_cast<int>(peerConsChanWord - 1u);
...
AICORE inline int EnsureOutgoingConsumerBinding(Pipe& pipe, uint32_t consId, uint32_t maxSpins)
{
    ...
    return prodChan == kGridInvalidChan ?
               prodChan :
               OpenDynamicOutgoingBinding(pipe, consId, prodChan, maxSpins);
}
    ...
    Pipe& pipe, uint32_t preferredProdId, int& acceptedChan)
{
    ...
    if (selectedProd == kGridInvalidChan) {
        ...
        for (int offset = 0; offset < peerCount; ++offset) {
            const uint32_t source =
                (start + static_cast<uint32_t>(offset)) % static_cast<uint32_t>(peerCount);
    ...
    acceptedChan = AcceptDynamicBindQueueRequest(
        pipe, requestProdId, token, static_cast<int>(peerProdChanWord - 1u), candidate);
    if (acceptedChan == kGridInvalidChan) {
        grid_mock::MockSetFault(FaultWord(pipe.closeScb[candidate]), grid_mock::kFaultBindProtocol);
        return DynamicBindQueueServiceResult::FAILED;
    }
    pipe.bindRequestScanStart =
        (static_cast<uint32_t>(selectedProd) + 1u) % static_cast<uint32_t>(peerCount);
...
AICORE inline int EnsureIncomingProducerBinding(Pipe& pipe, uint32_t prodId, uint32_t maxSpins)
{
    ...
    while (true) {
        ...
        const DynamicBindQueueServiceResult service =
            ServiceOneDynamicBindQueueRequest(pipe, prodId, acceptedChan);
