...
//
// The BcastSlotCount / GroupMax template params opt the pipe into the scheme-②
// TBROADCAST region appended after the unicast slot rings.  They default to 0, so a
// plain GridPipe<TileT, SlotBytes, SlotCount> (the unicast-only ReduceSum pipes)
// carries no broadcast state.
// ---------------------------------------------------------------------------
template <
...
// scoreboards: readyScb[c] is written by whichever core is bound as channel c's
// producer and freeScb[c] by its consumer, so kGridChanCount channels put twice
// that many distinct remote writers on what used to be a single line.  Monotone
// counters self-heal unless it is the LAST update that is lost, which is why the
// packed 4 B spacing stayed lucky for so long.
