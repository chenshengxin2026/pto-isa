...
ProgramFlatten::ProgramFlatten(ProgramAnalysis::ParamMap params) : analysis_(std::move(params)) {}
...
const std::vector<LinearProgramNode>& ProgramFlatten::flatten(const std::vector<ProgramNode>& program)
{
    linear_.clear();
    nextLoopId_ = 0;
    pc_ = 0;
...
void ProgramFlatten::visit(const std::vector<ProgramNode>& nodes, int64_t depth, const std::vector<int64_t>& loopStack)
{
    for (const auto& node : nodes) {
        if (node.kind == ProgramNode::Kind::Inst) {
...
void ProgramFlatten::emitInst(const ProgramInstNode& inst, int64_t depth, const std::vector<int64_t>& loopStack)
{
...
void ProgramFlatten::emitLoop(const ProgramLoopNode& loopNode, int64_t depth, const std::vector<int64_t>& loopStack)
{
