...
ProgramAnalysis::ProgramAnalysis(ParamMap params, std::unordered_map<std::string, ValueInfo> values)
...
void ProgramAnalysis::collectVregsFromInst(
    const ProgramInstNode& inst, std::unordered_map<std::string, bool>& vregs) const
{
...
void ProgramAnalysis::walkVregWarnings(
    const std::vector<ProgramNode>& nodes, const std::string& path, int64_t pregNum,
    ...
    for (const auto& node : nodes) {
        ...
        if (expandedVregNamespace > pregNum) {
            warnings.push_back(VregCapacityWarning{
                ...
                "Unroll-expanded virtual-register namespace exceeds physical register count. Prediction may be "
                "low-confidence for this case.",
            });
        }
...
std::vector<VregCapacityWarning> ProgramAnalysis::collectVregCapacityWarnings(
    const std::vector<ProgramNode>& program, int64_t pregNum) const
{
...
std::unordered_map<int, std::vector<int64_t>> ProgramAnalysis::inferTopBlockLoopBounds(
    const std::vector<ProgramNode>& program) const
{
