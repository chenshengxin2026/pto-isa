...
public:
    ...
    explicit ProgramAnalysis(ParamMap params = {}, std::unordered_map<std::string, ValueInfo> values = {});
    ...
    std::vector<VregCapacityWarning> collectVregCapacityWarnings(
    ...
    std::unordered_map<int, std::vector<int64_t>> inferTopBlockLoopBounds(
...
private:
    ...
    void collectVregsFromInst(const ProgramInstNode& inst, std::unordered_map<std::string, bool>& vregs) const;
    void walkVregWarnings(
